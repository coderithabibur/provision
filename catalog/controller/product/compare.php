		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				if ($product_info['image']) {
					...
					$imagePopup = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
				} else {
					$image = false;
					$imagePopup = '';
				}
				...
				$data['products'][$product_id] = array(
					...
					'price'        => $price,
					'special'      => $special,
				    'description'  => html_entity_decode($product_info['summary'], ENT_QUOTES, 'UTF-8'),
