	...
	public function save() {
		...
		if (!$json) {
			...
			if(!preg_match('/^[a-zA-Z\s]+$/',$this->request->post['firstname'])){
				$json['error']['firstname'] = $this->language->get('invalid_firstname');
			}
			...
			if(!preg_match('/^[a-zA-Z\s]+$/',$this->request->post['lastname'])){
				$json['error']['lastname'] = $this->language->get('invalid_lastname');
			}
			...
			if(!preg_match('/^[0-9]*$/', $this->request->post['telephone'])){
				$json['error']['telephone']  = $this->language->get('invalid_telephone');
			}

			if (utf8_strlen($this->request->post['fax']) >1){ 
				if(!preg_match('/^[0-9]*$/', $this->request->post['fax'])){
					$json['error']['fax'] = $this->language->get('invalid_fax');
				}
			}
