...
class ControllerExtensionCards extends Controller {
	...
	public function index() {
		...
		if (version_compare(VERSION, '4.1', '<')) {
			...
			$country_query = $this->db->query("(SELECT * FROM " . DB_PREFIX . "country c LEFT JOIN " . DB_PREFIX . "country_description cd ON (c.country_id = cd.country_id) WHERE c.country_id = " . $store_country . ") UNION (SELECT * FROM " . DB_PREFIX . "country c LEFT JOIN " . DB_PREFIX . "country_description cd ON (c.country_id = cd.country_id) WHERE c.country_id != " . $store_country . ")");
		}
