...
class ControllerExtensionPaymentStripe extends Controller {
	...
	public function createCheckoutSession() {
		...
		$checkout_data = array(
			...
			'cancel_url'				=> $this->url->link('checkout/cart', version_compare(VERSION, '4.0', '<') ? '' : 'language=' . $language, 'SSL'),
	...
	public function finalizePayment($payment_intent_id = '', $finalized_by_webhook = false) {
				...
				if (empty($customer_info['safe'])) {
					...
					foreach ($fraud_extensions as $extension) {
						...
						if ($fraud_status_id) {
							...
							$order_status_reason = 'Potential fraud';
						}
					}
				}
			}
		...
		if (!empty($settings['mismatch_status_id']) && $shipping_address != $payment_address) {
			...
			if ($settings['charge_mode'] == 'fraud') {
				$capture = false;
				$order_status_reason = 'Billing/shipping address mismatch';
			}
		}
		...
		// Check verifications
		if ($settings['review_status_id'] && isset($charge['outcome']['type']) && $charge['outcome']['type'] != 'authorized') {
			$order_status_id = $settings['review_status_id'];
			$order_status_reason = 'Stripe Radar recommendation is "Manual Review"';
		}
		
		if ($settings['elevated_status_id'] && isset($charge['outcome']['risk_level']) && $charge['outcome']['risk_level'] == 'elevated') {
			$order_status_id = $settings['elevated_status_id'];
			$order_status_reason = 'Risk status is "Elevated"';
		}
		
		if ($settings['highest_status_id'] && isset($charge['outcome']['risk_level']) && $charge['outcome']['risk_level'] == 'highest') {
			$order_status_id = $settings['highest_status_id'];
			$order_status_reason = 'Risk status is "Highest"';
		}
		
		if (isset($charge['payment_method_details']['card']['checks'])) {
			$checks = $charge['payment_method_details']['card']['checks'];
			
			if ($settings['street_status_id'] && $checks['address_line1_check'] == 'fail') {
				$order_status_id = $settings['street_status_id'];
				$order_status_reason = 'AVS street check failure';
			}
			
			if ($settings['postcode_status_id'] && $checks['address_postal_code_check'] == 'fail') {
				$order_status_id = $settings['postcode_status_id'];
				$order_status_reason = 'AVS postcode failure';
			}
			
			if ($settings['cvc_status_id'] && $checks['cvc_check'] == 'fail') {
				$order_status_id = $settings['cvc_status_id'];
				$order_status_reason = 'CVC check failure';
			}
		}
		
		if (!empty($charge['billing_details']['address']) && !empty($settings['mismatch_status_id'])) {
			if ($charge['billing_details']['address']['city'] != $payment_address['city']) {
				$order_status_id = $settings['mismatch_status_id'];
				$order_status_reason = 'Address mismatch (city)';
			}
			
			if ($charge['billing_details']['address']['postal_code'] != $payment_address['postcode']) {
				$order_status_id = $settings['mismatch_status_id'];
				$order_status_reason = 'Address mismatch (postcode)';
			}
		}
		...
		if ($payment_intent_id && empty($payment_intent['metadata']['cancel'])) {
			...
			if (!empty($payment_intent['status']) && $payment_intent['status'] != 'succeeded' && $payment_intent['status'] != 'requires_capture') {
				$comment .= $strong . 'Payment Status:</strong>' . $payment_intent['status'] . '<br>';
			}
		}
		
		if (!empty($order_status_reason)) {
			$comment .= $strong . 'Order Status Reason:</strong>' . $order_status_reason . '<br>';
		}
	...
	public function webhook() {
		...
		if (!isset($this->request->get['key']) || $this->request->get['key'] != md5($this->config->get('config_encryption'))) {
			$url_parts = explode('/', $this->config->get('config_url'));
			
			if (empty($url_parts[2]) || $this->request->server['HTTP_HOST'] != $url_parts[2]) {
				echo 'Wrong domain';
			} else {
				echo 'Wrong key';
		...
		if ($event['type'] == 'cash_balance.funds_available') {
			...
			if (empty($webhook['payment_intent']) || empty($webhook['metadata']['order_id'])) return;
			...
			if ($this->request->server['HTTP_HOST'] != $store_domain && strpos($this->request->server['HTTP_HOST'], 'ngrok') === false) return;
