...
class ControllerFeedUksbGoogleMerchant extends Controller {
	public function index() {
		...
		if ($this->config->get('uksb_google_merchant_status')) { 
			...
			if(!isset($this->request->get['mode']) || ($this->request->get['mode']=='cron' && $split[0] == 1)){
				...
				$output .= '<?xml-stylesheet type="text/xsl" href="xsl-stylesheets/uksb_google_merchant.xsl"?>' . "\n";
			...
			foreach ($products as $product) {
				...
				if($variant==1){
					for($i = 1; $i <= $max_variants; $i++){
						...
						if(isset($images[$j]) && $images[$j] != 'no_image.jpg' && trim($images[$j]) != '' ){
						...
						if($product['g_is_bundle']){
							$output .= '<g:is_bundle>yes</g:is_bundle>'."\n";
						}
						
						if($product['g_expiry_date']){
							$output .= '<g:expiry_date>' . $product['g_expiry_date'] . '</g:expiry_date>'."\n";
						}
						...
						if(isset($colours[$j])&&trim($colours[$j])!=''){
							$output .= '<g:color><![CDATA[' . trim($colours[$j]) . ']]></g:color>'."\n";
						}
						...
						if($product['g_promotion_id']!=''){
							$output .= '<g:promotion_id><![CDATA[' . $product['g_promotion_id'] . ']]></g:promotion_id>'."\n";
						}
					...
					if($product['g_colour']){
						$output .= '<g:color><![CDATA[' . $product['g_colour'] . ']]></g:color>'."\n";
					}
			...
			if(isset($this->request->get['mode'])){
				...
				if($this->model_feed_uksb_google->getTotalProducts() > $split[1]){
					$this->response->redirect('index.php?route=feed/uksb_google_merchant&mode=cron&language='.$lang.'&currency='.$curr.'&send='.($split[0]+$cronlimit).'-'.($split[1]+$cronlimit));
				}
			}
		}
	}
	...
	public function serverURL(){
		...
		$url_parts = explode("/", HTTP_SERVER);
		$subfolder = (!empty($url_parts[3]) ? $url_parts[3].'/' : '');

		return ($isSecure ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/' . $subfolder;
	}
