...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
			$other_directory = rtrim(DIR_IMAGE . 'cache/catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'catalog';
		}
		...
		if (!$json) {
			...
			/*$this->load->model('tool/image');
            $this->model_tool_image->resize($directory . '/' . $filename, 100, 100);
			
			
			$filename_array = explode(".", $filename);
			$filename1 = $filename_array[0].'-100x100.'.$filename_array[1];
			copy($directory . '/' . $filename, $other_directory . '/' . $filename1);
			$filename2 = $filename_array[0].'-108x108.'.$filename_array[1];
			copy($directory . '/' . $filename, $other_directory . '/' . $filename2);
			$filename3 = $filename_array[0].'-500x500.'.$filename_array[1];
			copy($directory . '/' . $filename, $other_directory . '/' . $filename3);
			//$this->resize_image($directory.'/'.$filename, 100, 100);
			*/

			$json['success'] = $this->language->get('text_uploaded');
			$json['temp_name'] = $this->request->files['file']['tmp_name'];
			$json['directory'] = $directory;
			$json['image_name'] = $filename;
		}
