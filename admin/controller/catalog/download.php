...
class ControllerCatalogDownload extends Controller {
	...
	public function subscribeSheet(){
		$this->language->load('catalog/download');
		$this->load->model('catalog/download');
		$results = $this->model_catalog_download->getsubscribeSheet();
		
		$filename = 'data' . time();  
        $sep = "\t"; //tabbed character
        // excels header info for browser
		...
		foreach($results as $res): 
			$schema_insert = ""; 
			$schema_insert .= $res['id']. $sep;
			$schema_insert .= $res['email'].  $sep;
			$schema_insert .= $res['currebt_date'].  $sep;
