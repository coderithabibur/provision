...
class ModelCatalogSitemap extends Model {	
	public function generate() {
		...
		if(count($length) == 1) {
			...
			fwrite($fp, $this->getCategories($on,HTTP_CATALOG));
			fwrite($fp, $this->getProducts(HTTP_CATALOG));
			fwrite($fp, $this->getInformationPages(HTTP_CATALOG));
			fwrite($fp, $this->getManufacturers(HTTP_CATALOG));
		} else {
			fwrite($fp, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
			fwrite($fp, "<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\" \r\n xmlns:image=\"http://www.google.com/schemas/sitemap-image/1.1\" \r\n  xmlns:xhtml=\"http://www.w3.org/1999/xhtml\">\r\n");
			fwrite($fp, $this->getMGeneral($server));
			fwrite($fp, $this->getMCategories($length,$on,HTTP_CATALOG));
			fwrite($fp, $this->getMProducts($length,HTTP_CATALOG));
			fwrite($fp, $this->getMInformationPages($length,HTTP_CATALOG));
			fwrite($fp, $this->getMManufacturers(HTTP_CATALOG));
		}
		fwrite($fp, "</urlset>");
		fclose($fp);
	}
