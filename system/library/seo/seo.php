...
final class Seo {
	...
	public static function findGeneralTotal($registry) {
		...
		$sql = "SELECT id FROM `" . DB_PREFIX . "seo_data` WHERE `type` = 'general'";
	...
	public static function findProducts($registry, $data = array()) {
		...
		return $object_array;
						...
						public static function findBlogs($registry, $data = array()) {

		$db = $registry->get('db');
		$config_language_id = $registry->get('config')->get('config_language_id');

        $sql = "SELECT p.blog_id FROM `" . DB_PREFIX . "magikblog` p INNER JOIN `".DB_PREFIX."magikblog_description` pd ON (p.`blog_id` = pd.`blog_id`) WHERE pd.language_id = '" . (int)$config_language_id . "' ";
		...
		foreach($query->rows as $row) {
			$object_array[] = self::getObject('blog', $row['blog_id'], $registry);
		}
	...
	public static function findBlogsTotal($registry, $data = array()) {
