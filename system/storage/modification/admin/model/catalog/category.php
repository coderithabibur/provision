...
$data['category_description'] = array();
		...
		foreach ($data['category_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', refine_search_text = '" . $this->db->escape($value['refine_search_text']) . "', product_listing_text = '" . $this->db->escape($value['product_listing_text']) . "'");
		}
	...
	public function editCategory($category_id, $data) {
		// START: NitroPack.io (https://nitropack.io)
                $this->event->trigger('pre.admin.category.edit.nitropack', $category_id);
		...
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', image = '" . $this->db->escape($data['image']) . "', icon = '" . $this->db->escape($data['icon']) . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
	...
	public function deleteCategory($category_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "uksb_google_merchant_categories WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategoryDescriptions($category_id) {
		...
		foreach ($query->rows as $result) {
			$category_description_data[$result['language_id']] = array(
				...
				'refine_search_text' 	=> $result['refine_search_text'],
				'product_listing_text' 	=> $result['product_listing_text']
			);
		}
