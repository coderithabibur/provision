...
class ControllerCommonMenu extends Controller {
	public function index() {
			...
			if($this->user->hasPermission('access', 'extension/bganycombi') && file_exists('model/extension/bganycombi.php')) { 
				$data['bganycombi'] = $this->url->link('extension/bganycombi', 'token=' . $this->session->data['token'], 'SSL');	
			}
		...
		$data['analytics'] = $this->url->link('extension/analytics', 'token=' . $this->session->data['token'], 'SSL');
			...
			$data['mpimportexport_menu'] = $this->load->controller('module/mpimportexport/getMenu');
		...
		$data['error_log'] = $this->url->link('tool/error_log', 'token=' . $this->session->data['token'], 'SSL');
				...
				$data['apps_link'] = $this->url->link('extension/hbapps', 'token=' . $this->session->data['token'], 'SSL');
				$data['seo_link'] = $this->url->link('extension/hbseo', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['report_subscribes'] = $this->url->link('catalog/download/subscribeSheet', 'token=' . $this->session->data['token'], 'SSL');
		...
		// START: NitroPack.io (https://nitropack.io)
                if (is_file(DIR_CONFIG . 'nitropackio/compatibility.php')) {
                    ...
                    $data['has_nitropack_access'] = $this->user->hasPermission('access', $nitropack_config['route']['module']['nitropack']);

                    $this->load->model($nitropack_config['route']['module']['nitropack']);

                    $data['nitropack_stores'] = array();

                    foreach ($this->{$nitropack_config['model']['module']['nitropack']}->getStores() as $store) {
                        array_push($data['nitropack_stores'], array(
                            'name' => $store['name'],
                            'href' => $this->url->link($nitropack_config['route']['module']['nitropack'], 'token=' . $this->session->data['token'] . '&store_id=' . $store['store_id'], true)
                        ));
                    }
                }
                // END: NitroPack.io (https://nitropack.io)

			// Product Questions Menu
			$this->load->language('module/product_question_menu');
			
			$data['text_menu_product_question'] = $this->language->get('text_menu_product_question');
			$data['text_menu_setting'] = $this->language->get('text_menu_setting');
			$data['text_menu_question'] = $this->language->get('text_menu_question');
			$data['text_menu_help'] = $this->language->get('text_menu_help');
			
			$data['product_question_setting'] = $this->url->link('module/product_question', 'token=' . $this->session->data['token'], true);
			$data['product_question_question'] = $this->url->link('catalog/question', 'token=' . $this->session->data['token'], true);
			$data['product_question_help'] = 'https://www.oc-extensions.com/OpenCart-Product-Questions-Answers-FAQ-OpenCart-2.x-Help';
			// Stop Product Question Menu
