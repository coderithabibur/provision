...
if($this->config->get('config_redirect_status')) {
          ...
          $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "redirect_manager WHERE fromUrl = '" .$this->db->escape($url1)."' and status = 1");
          if($query->num_rows) {
            $this->db->query("UPDATE " . DB_PREFIX . "redirect_manager SET times_used = times_used + '1' WHERE fromUrl = '" .$this->db->escape($url1)."'");
