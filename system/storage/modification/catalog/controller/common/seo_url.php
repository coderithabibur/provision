	...
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			if (isset($this->request->get['product_id']) && isset($this->request->get['path'])) {
				...
				foreach($results as $key => $value){
					$this->session->data['hreflang'][$value['language_id']] = $this->nerdherd($server."index.php?route=".$this->request->get['route'],$value['language_id'],$server);
			}
		...
		foreach ($data as $key => $value) {
		 ...
		 foreach ($data as $key => $value) {
				...
				if (isset($data['route'])) {
				    ...
				    if($key = 'route') {
				    	$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "url_alias` WHERE `query` = 'route=" . $this->db->escape($value) . "' AND lang = '".(int)$langid."'");
				    	if($query->num_rows) {
				    		$url .= '/' . $query->row['keyword'];
				    	}
			        }				
		...
		foreach ($data as $key => $value) {
		 ...
		 foreach ($data as $key => $value) {
				...
				if (isset($data['route'])) {
				    ...
				    if($key = 'route') {
				    	$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "url_alias` WHERE `query` = 'route=" . $this->db->escape($value) . "' AND lang = '".(int)$this->config->get('config_language_id')."'");
				    	if($query->num_rows) {
				    		$url .= '/' . $query->row['keyword'];
				    	}
			        }				
