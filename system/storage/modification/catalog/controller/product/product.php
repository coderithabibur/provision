	...
	public function index() {
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			$data['breadcrumbs'][] = array(
				...
				if($this->config->get("nerdherd_breadcrumblink") || $this->config->get("nerdherd_manu_breadcrumblink")) {
					$data['breadcrumbs'] = array();
				...
				if($this->config->get("nerdherd_manu_breadcrumblink")) {
					
					$breadmanu =  $this->model_catalog_product->getBreadManu($this->request->get['product_id']);
					if (!empty($breadmanu)) {
						$data['breadcrumbs'][] = array(
							'text'      => $breadmanu['name'],
							'href'      => $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $breadmanu['manufacturer_id']),
							'separator' => $this->language->get('text_separator')
						);
					}
...
//        echo "<pre>";print_r($product_info);exit;
		if ($product_info) {
			...
			$this->document->addScript('catalog/view/javascript/jquery/magnific/jquery.touchSwipe.min.js');
            ...
            $data['video_overview'] = html_entity_decode($product_info['video_overview'], ENT_QUOTES, 'UTF-8');
			...
			$data['button_addtocompare'] = $this->language->get('button_addtocompare');
			...
			if(!empty($product_info['txt_each'])){
			...
			$data['spec']  = html_entity_decode($product_info['summary'], ENT_QUOTES, 'UTF-8');
			...
			if ($product_info['image']) {
				...
				if ($product_info['image']) {
					$data['popuppin'] = $this->model_tool_image->resize($product_info['image'], $this->language->get('google_image_width'), $this->language->get('google_image_height'));
				} else {
					$data['popuppin'] = '';
				}
				...
				$this->session->data['pmodel'] = $product_info['model'];
				$this->session->data['pbrand'] = $product_info['manufacturer'];
			...
			foreach ($results as $result) {
				...
				if($result['reviews']) {
					...
					$reviews = sprintf($this->language->get('text_reviews'), '0');
				}
	...
	public function review() {
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($review_total) ? (($page - 1) * 4) + 1 : 0, ((($page - 1) * 4) > ($review_total - 4)) ? $review_total : ((($page - 1) * 4) + 4), $review_total, ceil($review_total / 4));
