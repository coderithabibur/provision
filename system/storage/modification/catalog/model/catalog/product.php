...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
                ...
                if (__FUNCTION__ == 'getProduct' && is_file(DIR_CONFIG . 'nitropackio/compatibility.php')) {
                    $this->load->config('nitropackio/compatibility');
                    $nitropack_config = $this->config->get('nitropackio');
                    $nitropack_action = new \Action($nitropack_config['route']['module']['nitropack'] . '/afterGetProduct', $query->row['product_id']);
			...
			return array(
                ...
                'video_title'      => $query->row['video_title'],
                'video_overview'   => $query->row['video_overview'],
...
public function getBreadCategories($product_id) {
		...
		public function getBreadManu($product_id) {
			$return_data = array();
			$query = $this->db->query("SELECT p.manufacturer_id,m.name FROM " . DB_PREFIX . "product p LEFT JOIN  " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id)  WHERE p.product_id = '" . (int)$product_id . "' LIMIT 1 ");
			
			if($query->num_rows == 1){
				$return_data['name'] = $query->row['name'];
				$return_data['manufacturer_id'] = $query->row['manufacturer_id'];
			}
