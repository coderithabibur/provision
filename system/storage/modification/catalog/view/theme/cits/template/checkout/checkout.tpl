            ...
            if (json['redirect']) {
                ...
                $.ajax({
                    ...
                    success: function(html) {
                        $('#collapse-payment-address .panel-body').html(html);
               $('html, body').animate({
        scrollTop: $('#collapse-payment-address').offset().top
    }, 'slow');
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                if (json['error']['warning']) {
                    $('#collapse-payment-method .buttons').prepend('<div class="alert alert-warning">' + json['error']['warning'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                }
            } else {
                $.ajax({
    ...
    $('html, body').animate({
        scrollTop: $('.pageTitle').offset().top
			    ...
			    <!--START - GOOGLE ANALYTICS ECOMMERCE TRACKING-->
			        <?php if (isset($ecommerce_checkout_script)) { echo $ecommerce_checkout_script; } ?> 
